...
function AC9Pill({ code }: { code: string }) {
  return (
    <span style={{ display: "inline-block", background: "#6366f1", color: "#fff", padding: "2px 10px", borderRadius: 20, fontSize: 10, fontWeight: 700, letterSpacing: "0.04em", fontFamily: "monospace" }}>
      {code}
    </span>
  );
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.12em", color: "#6366f1", marginBottom: 10 }}>
      {children}
    </div>
  );
}
...
function PhaseIcon({ name }: { name: string }) {
  ...
  return <span style={{ fontSize: 12, fontWeight: 700, color: "#6366f1" }}>{icons[name] || "L"}</span>;
}
                  ...
                  <thead>
                    <tr style={{ background: "#f5f3ff" }}>
                      {["Phase", "Duration", "Teacher", "Students"].map((h, i) => (
                        <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6366f1", borderBottom: "2px solid #e0e7ff", ...(i === 0 ? { width: 130 } : {}) }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            {result && (
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button onClick={() => download("pdf")} style={{ padding: "6px 14px", background: "#6366f1", color: "#fff", border: "none", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>PDF</button>
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {result ? (
              ...
              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 20, textAlign: "center", paddingTop: 80, paddingBottom: 80, boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
                <div style={{ background: "rgba(99,102,241,0.15)", borderRadius: 10, padding: "10px 14px", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
